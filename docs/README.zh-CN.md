...
整个架构把三条不能合并成自治循环的链路显式分开，并保留独立的本地 Provenance：
...
仍不是无损优化：它会降低统计把握度，只能标为 Preliminary 或 Inconclusive。成本安全的做法是把
无关的确定性 Release 标为 `NOT_APPLICABLE`，而不是运行削弱版 Gate；一旦版本采纳 Suite 实际评估的
精确治理 Candidate，或声明 Suite 覆盖的 Agent 行为发生变化，完整 Gate 仍然是强制项。声明式
Skill/API 契约只有在每条行为敏感变更路径都有经审阅的例外理由和确定性 Proof 时才能归为
deterministic；这种分类不构成真实行为声明。
...
| --- | --- | --- |
| `deterministic` | 必须省略 | 以理由记录 `NOT_APPLICABLE`；全量测试、业务 Fixture、Audit、构建与 Wheel Smoke Test 仍全部执行。 |
...
都会发布分类契约、绑定 Commit 的验证结果与 `release-evidence.json`；governance Release 还会长期
